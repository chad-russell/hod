...
- `hod run` and `hod shell` accept a recipe specifier (64-char hex hash or path to a `.ts` file). For `.ts` files, they evaluate with `bun run`, build if needed, then exec the command or spawn a shell.
- `hod import-file` and `hod resolve` are not current CLI commands.
...
- `Unpack` recipes are fully built — tar extraction via the system `tar` binary.
