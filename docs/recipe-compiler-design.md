...
Three new subcommands for the `hod` binary. These are thin wrappers around existing functionality (serde JSON ↔ binary encode/decode, BLAKE3 hashing).
...
- Encodes it to binary `.hod` format using the existing `Recipe::decode_json` → `Recipe::encode` path
...
const bash = await process({
  ...
  dependencies: [
    ...
    dep("seed", "../bootstrap/seed-root.hod"),
  ],
  runtime_deps: ["glibc"],
});
```
...
- A path to an existing `.hod` or `.json` file (the library reads and resolves it)
...
- A string — treated as a hex hash (for hardcoded/known hashes)
- A path string ending in `.hod` or `.json` — the library shells out to resolve it

```typescript
dep("glibc", glibcRecipe)           // BuiltRecipe from import
dep("seed", seedRecipe)             // BuiltRecipe from import
dep("source", "../bash-source.hod") // path to existing file
...
Reads an existing `.hod` file, shells out to `hod decode` to get the JSON, then to `hod encode` to get the hash. Returns `Promise<BuiltRecipe>`.
...
#### `importToStore(recipe, storePath?)`

Imports the recipe into the hod store. Equivalent to `hod import-recipe <hod-file>`. Returns the hash.

```typescript
importToStore(bash);
```

*(Note: `hod import-recipe` may not exist yet — this is a placeholder for a store-import command.)*
...
```typescript
// recipes/build.ts
import { importToStore } from "hod-sdk";
import { glibcRecipe } from "./glibc/glibc.ts";
import { bashRecipe } from "./bash/bash.ts";
import { gccRecipe } from "./gcc/gcc.ts";

// Import all to store
importToStore(glibcRecipe);
importToStore(bashRecipe);
importToStore(gccRecipe);

console.log("All recipes built and imported.");
```
...
│   │   ├── dep.ts          # dep() helper
│   │   ├── encode.ts       # writeHod(), writeJson(), importToStore()
...
| `hod import-recipe` command | TBD | May not exist yet. Needed for `importToStore()`. Could be `hod encode --import` or a separate command. |
| Download recipe support | Deferred | Start with Process + File. Add `download()` constructor when needed. |
| Directory / Symlink / Unpack recipes | Deferred | Add constructors when needed. |
