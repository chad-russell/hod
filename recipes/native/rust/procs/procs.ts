//! procs native build recipe — modern replacement for ps.
//!
//! Builds procs 0.14.11 from source using the prebuilt Rust toolchain.
...
//!
//! procs uses `edition = "2024"` and `rust-version = "1.88"`, both
//! satisfied by Rust 1.95.0.
//!
//! Default features are enabled (includes Docker/container awareness
//! via the `docker` feature, which pulls in `dockworker` + `tokio`).
//!
//! ## Note on hermeticity
//!
...
//!
//! procs is a single dynamically-linked binary that needs the C toolchain's
...
export const procsRecipe = recipe;
