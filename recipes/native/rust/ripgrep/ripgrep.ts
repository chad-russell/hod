//! ripgrep native build recipe — fast line-oriented search tool.
//!
//! Builds ripgrep 15.1.0 from source using the prebuilt Rust toolchain.
...
//!
//! ripgrep's Cargo.toml uses `edition = "2024"` and `rust-version = "1.85"`,
...
//!
//! ripgrep is a single dynamically-linked binary that only needs the C
...
const recipe = await cargoBuild({
  ...
  source: "source",
  deps: [
    dep("source", ripgrepSourceRecipe),
...
export const ripgrepRecipe = recipe;
