//! gcc-stage2 — Full C/C++ native compiler built by gcc-stage1, running on glibc.
//!
//! This extends gcc-stage2-c with C++ support:
//!   build  = x86_64-linux-gnu  (glibc runtime now works in sandbox)
//!   host   = x86_64-linux-gnu  (runs on glibc)
//!   target = x86_64-linux-gnu  (produces glibc-targeting code)
//!
//! After this, the native-toolchain bundle can use gcc-stage2 as the
//! sole compiler dependency for downstream packages.
...
# === Set up GCC internal headers at the baked-in paths ===
...
if [ -x $OUT/bin/x86_64-linux-gnu-g++ ]; then
  ...
  env: [
    // Empty C_INCLUDE_PATH — CC_FOR_BUILD gets seed (musl) headers via -I flag.
    // CC gets glibc headers via --sysroot and -isystem. Setting C_INCLUDE_PATH
    // globally would contaminate the cross-compiler with musl's stdarg.h etc.
...
export const gccStage2Recipe = recipe;
