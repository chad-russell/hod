...
// ===========================================================================
// Test 3: Packed output structure is correct (Bootstrap mode)
// ===========================================================================

#[test]
#[ignore]
fn packed_output_structure_is_correct() {
    let tmp = tempfile::tempdir().unwrap();
    let store_path = tmp.path();

    // Build validate-stage1 first to populate the store
    let output = hod_build(
        "recipes/cross/validate-stage1.hod",
        store_path,
    );
    ...
    // Build glibc-runtime
    let glibc_rt_output = hod_build(
        "recipes/cross/glibc-runtime.hod",
        store_path,
    );
    assert!(
        glibc_rt_output.status.success(),
        "glibc-runtime should build: {}",
        String::from_utf8_lossy(&glibc_rt_output.stderr)
    );

    // Build the packed executable
    let packed_output = hod_build(
        "recipes/cross/hello-packed.hod",
        store_path,
    );
    assert!(
        packed_output.status.success(),
        "hello-packed should build: {}",
        String::from_utf8_lossy(&packed_output.stderr)
    );

    let packed_stdout = String::from_utf8_lossy(&packed_output.stdout);
    let packed_hash = packed_stdout.trim().lines().last().unwrap().to_string();

    // Verify packed output structure
    let packed_staging = {
        let parsed = hex_to_hash(&packed_hash).unwrap();
        let shard = hod::hash::hash_shard(&parsed);
        store_path.join("staging").join(&shard).join(&packed_hash)
    };

    assert!(packed_staging.exists(), "packed output staging should exist");
    assert!(packed_staging.is_dir(), "packed output should be a directory");
    assert!(
        packed_staging.join("bin").exists(),
        "packed output should have bin/"
    );
    assert!(
        packed_staging.join("lib").exists(),
        "packed output should have lib/"
    );

    // Verify the binary is in bin/
    let binary_path = packed_staging.join("bin/binary");
    assert!(binary_path.exists(), "packed binary should exist at bin/binary");

    // Verify the binary has AT_EXECFN bootstrap injected
    let packed_binary = std::fs::read(&binary_path).unwrap();
    assert!(hod::packed::is_elf(&packed_binary), "packed binary should be valid ELF");

    // After bootstrap injection, PT_INTERP should be gone (converted to PT_LOAD)
    let parsed = goblin::elf::Elf::parse(&packed_binary).expect("should parse packed binary");
    let has_interp = parsed.program_headers.iter().any(|p| {
        p.p_type == goblin::elf::program_header::PT_INTERP
    });
    assert!(
        !has_interp,
        "PT_INTERP should have been replaced by AT_EXECFN bootstrap"
    );

    // Verify the dynamic linker is in lib/
    assert!(
        packed_staging.join("lib/ld-linux-x86-64.so.2").exists(),
        "lib/ should contain the dynamic linker"
    );

    // Verify RPATH was patched to $ORIGIN/../lib
    let rpath_info = hod::packed::find_rpath(&packed_binary);
    match rpath_info {
        Ok(hod::packed::RpathInfo::Rpath { offset, len }) => {
            let rpath = std::str::from_utf8(
                &packed_binary[offset as usize..offset as usize + len - 1]
            ).unwrap();
            assert!(
                rpath.starts_with("$ORIGIN"),
                "RPATH should be relative ($ORIGIN-based), got: {rpath}"
            );
        },
        Ok(hod::packed::RpathInfo::Runpath { offset, len }) => {
            let runpath = std::str::from_utf8(
                &packed_binary[offset as usize..offset as usize + len - 1]
            ).unwrap();
            assert!(
                runpath.starts_with("$ORIGIN"),
                "RUNPATH should be relative ($ORIGIN-based), got: {runpath}"
            );
        },
        Ok(hod::packed::RpathInfo::Absent) => {
            // This is OK if the binary had no RPATH to patch
        },
        Err(_) => {},
    }
}
